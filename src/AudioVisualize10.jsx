...
function AudioSphere10() {
    ...
    useEffect(() => {
        ...
        //const bloomEffect = new BloomEffect("bloom", new BABYLON.PostProcessRenderEffect);
        ...
        function animateSphere(bpm, icosphere) {
            ...
            const animations = ['x', 'y', 'z'].map(axis => {
                ...
                const keyFrames = [
                    { frame: 0, value: 0.7 },
                    { frame: totalFrames * 0.1 / 2, value: 1.6 },
        ...
        const setupIcoSpheres = async () => {
            const icospherePromises = musicPaths.map(async (path, index) => {
                ...
                const icosphere = MeshBuilder.CreateSphere(`sphere${index}`, { diameter: 7 }, scene);
                ...
                //icosphere2 is a bpm animating sphere
                const icosphere2 = MeshBuilder.CreateIcoSphere(`sphere${index}`, { radius: 3, subdivisions: 6 }, scene);
                //const icosphere2 = MeshBuilder.CreateSphere(`sphere${index}`, { diameter: 6 }, scene);
                ...
                gl.addIncludedOnlyMesh(icosphere);
                gl.addIncludedOnlyMesh(icosphere2);
